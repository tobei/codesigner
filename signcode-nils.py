...
jarSignerPath = pathlib.Path('C:/Program Files/Java/jdk1.8.0_281/bin/jarsigner.exe')
netsignerPath = pathlib.Path('C:/Program Files (x86)/Windows Kits/10/bin/10.0.19041.0/x86/signtool.exe')

tsa = "http://timestamp.digicert.com"
proxyHost = "-J-Dhttp.proxyHost=proxyusr.fediap.be"
proxyPort = "-J-Dhttp.proxyPort=8080"
...
def sign_dotnet_library(library, name):
    ...
    if result.returncode is not 0:
        ...
        result = subprocess.run([str(netsignerPath), "timestamp", "/tr", tsa, "/td", "sha256", library.name], capture_output=True)
        assert_external_toolresult(name, result, "")
        result = subprocess.run([str(netsignerPath), "verify", "/pa", library.name], capture_output=True)
        assert_external_toolresult(name, result, "")
    else:
